...
基于现有 LangGraph 基础设施扩展，构建一个 SuperGraph。复用现有 `src/lib/run-runtime/` 的 GraphRun/GraphStep/GraphStepAttempt 模型进行运行记录和断点续跑，SuperGraph 通过 `run-runtime/graph-executor.ts` 执行：
...
- 步骤：风格定义 → 角色形象生成 → 场景概念图 → VLM 自洽检查 → 不合格重试（每项最多 3 次）
...
- 步骤：分镜规划 → Prompt 合成 → 批量生图 → 一致性校验 → 不合格重试（每 Panel 最多 3 次）
...
2. 场景匹配 — 背景环境是否匹配场景描述
3. 风格一致 — 画风是否与全局风格一致
4. 评分决策（默认阈值 0.7，可在 PipelineRun.config 中调整）：score ≥ 0.7 → 通过；score < 0.7 且 retries < 3 → 调整 Prompt 重试；retries ≥ 3 → 标记待人工审核。VLM 校验使用项目已配置的 LLM provider（需支持视觉输入），通过现有 text.worker 执行
